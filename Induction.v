...
(** **** Exercise: 1 star, standard, optional (destruct_induction)
    ...
    destruct is used to deal with types with limited possible values, 
    and divides the proof into multiple sub-goals, each sub-goal corresponds
    to a value situation. Induction is used to deal with the type of inductive
    definition, and the proof is divided into two sub-goals, each sub-goal 
    corresponds to the base case and the induction step, and the inductive 
    hypothesis needs to be used to handle the incremental case.
...
Qed.
    ...
    - First, suppose [n = 0].  We must show that
      ...
      This follows directly from the definition of plus_n_O.
    ...
    - Next, suppose [n = S n'], where
      ...
      which is immediate from the induction hypothesis and plus_n_Sm .
...
Theorem n_eqn_n: forall n , (n =? n) = true.
Proof.
  intros n. induction n as [|n' IHn'].
  - simpl. reflexivity.
  - simpl. rewrite IHn'. reflexivity.
Qed. 
    ...
    - First, suppose [n = 0].  We must show that
      ...
      This follows directly from the definition of eqb.
    ...
    - Next, suppose [n = S n'], where
      ...
      By the definition of eqb, this follows from
...
Theorem plus_assoc_reverse : forall n m p:nat,
  n + (m + p) = (n + m) + p.
Proof.
  intros n m p. induction n as [|n' IHn'].
  -simpl. reflexivity.
...
Proof.
  intros n m p.
  assert (n + (m + p) = (n + m) + p) as H1. (* 引入中间结果 H1 *)
  { rewrite  plus_assoc_reverse. reflexivity. } (* 证明中间结果 H1 *)

  assert (m + (n + p) = (m + n) + p) as H2. (* 引入中间结果 H2 *)
  { rewrite  plus_assoc_reverse. reflexivity. } (* 证明中间结果 H2 *)
  
  assert (n + m = m + n) as H3. 
  {rewrite -> add_comm. reflexivity. }
  rewrite ->H1. rewrite -> H2. rewrite -> H3. (* 将中间结果代入目标中 *)
  reflexivity.
Qed.
(* Proof.
  intros n m p. induction n as [|n' IHn'].
...
Proof.
  intros n. induction n as [|n' IHn'].
  - simpl. reflexivity.
  - simpl. assert (H: n' + 0 = n'). { rewrite -> plus_n_O. reflexivity. } rewrite H. reflexivity.
Qed.
...
Proof.
  ...
  -assert (Hn: n * 0 = 0). { rewrite mul_0_r. reflexivity. } rewrite Hn.
   assert (Hm: m * 0 = 0). { rewrite mul_0_r. reflexivity. } rewrite Hm.
   simpl. assert (Hmn: ((n + m) * 0 = 0)). { rewrite mul_0_r. reflexivity. } rewrite Hmn.
   reflexivity.
  -assert (Hn: n * S p' = n * p' + n). { rewrite <- mult_n_Sm. reflexivity. } rewrite Hn.
   assert (Hm: m * S p' = m * p' + m). { rewrite <- mult_n_Sm. reflexivity. } rewrite Hm.
   assert (Hmn: ((n + m) * S p' = (n + m) * p' + (n + m))). { rewrite <- mult_n_Sm. reflexivity. } rewrite Hmn.
   rewrite IHp'.
   assert (Hadd_assoc: (n * p' + m * p' + (n + m) = n * p' + m * p' + n + m)). { rewrite add_assoc. reflexivity. } rewrite Hadd_assoc.
   assert (Hadd_assoc2: (n * p' + n + (m * p' + m) = n * p' + n + m * p' + m)). { rewrite add_assoc. reflexivity. } rewrite Hadd_assoc2.
   assert (Hadd_comm: m * p' + n = n + m * p'). { rewrite add_comm. reflexivity. } rewrite Hadd_comm. 
   reflexivity.
Qed.
*)
Theorem mult_plus_distr_r : forall n m p : nat,
...
Proof.
  intros b. induction b as [|n1 IHn1| n2 IHn2].
  -simpl. reflexivity.
  -simpl. reflexivity.
  -simpl. rewrite -> IHn2. simpl. rewrite -> add_comm. simpl. rewrite -> add_comm. reflexivity.
Qed.
...
Example test_bin_nat_bin : nat_to_bin(bin_to_nat (B0(B1 Z))) = (B0 (B1 (B0 Z))).
  simpl.
Abort.

(*
Theorem bin_nat_bin : forall b, nat_to_bin (bin_to_nat b) = b.
Proof.
  intros b. induction b as [|b0' IHb0'|b1' IHb1'].
  - simpl. reflexivity.
  - simpl. 
...
(*周康祺*)
Theorem bin_nat_bin_auxiliary : forall b , nat_to_bin (double b) = double_bin (nat_to_bin b).
Proof.
  induction b.
  - simpl. reflexivity.
...
Proof.
  induction b.
  - simpl. reflexivity.
  - simpl. rewrite <- IHb. assert (H: bin_to_nat b + 0 = bin_to_nat b). 
    { simpl. rewrite -> add_0_r. reflexivity. } rewrite -> H. 
    rewrite <- bin_nat_bin_auxiliary. rewrite <- double_plus. reflexivity.
  - simpl. rewrite <- IHb. assert (H: bin_to_nat b + 0 = bin_to_nat b). 
    { simpl. rewrite -> add_0_r. reflexivity. } rewrite -> H. 
    rewrite <- bin_nat_bin_auxiliary. rewrite -> double_plus. reflexivity.
Qed. *)
